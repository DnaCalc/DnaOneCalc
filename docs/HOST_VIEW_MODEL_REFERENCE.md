...
**`ExtensionSurfaceState`** (`state/types.rs`) — currently carries `admitted_contract: FrozenExtensionHostContract`, `provider_catalog: ExtensionProviderCatalog`, and `rtd_host: ExtensionRtdHostState`. The host state freezes the admitted OneCalc extension ABI v0 contract into runtime-owned truth, keeps a platform-gated native-provider loader catalog alongside it, and now also carries the host-owned RTD lifecycle state. `provider_catalog` preserves the lifecycle distinction between `discovered`, `admitted`, `enabled`, `active`, `blocked-by-platform`, and `rejected-by-contract` providers. `rtd_host` preserves the declared topic/request lifecycle (`activation-pending`, `awaiting-value`, `value-ready`, `update-pending`, `provider-not-ready`, `blocked-by-platform`, `disconnected`) over the runtime-specific RTD host contract (`in-process-com`, `minimal-com-like-registry`, or `unsupported`). Packaging parity proof for Windows `.xll` and Linux `.so` now lives in the executable harnesses under `extensions/packaging_conformance.rs`, which assert that both packaging lanes preserve the same admitted ABI claims even though the binary formats differ.
