...
**`ExtensionSurfaceState`** (`state/types.rs`) — currently carries `admitted_contract: FrozenExtensionHostContract` plus `provider_catalog: ExtensionProviderCatalog`. The host state now freezes the admitted OneCalc extension ABI v0 contract into runtime-owned truth and keeps a platform-gated loader catalog alongside it. `provider_catalog` records the current runtime platform gate (`windows-desktop`, `linux-desktop`, or `browser-wasm`) and preserves the lifecycle distinction between `discovered`, `admitted`, `enabled`, `active`, `blocked-by-platform`, and `rejected-by-contract` providers. This is the host-owned extension load-state reporting surface that later Extension Center UI work will read.
