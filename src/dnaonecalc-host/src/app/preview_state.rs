...
/// Seed for the WS-14 minimal home shell — a single empty `untitled-1`
/// formula space, no retained-artifact catalog, no demo-mode hints. Used
/// by `mount_onecalc_preview` after the WS-14 mount switch.
///
/// The legacy three-mode `preview_host_state()` (which seeds three demo
/// scenarios + matched/blocked/mismatched retained artifacts) is preserved
/// alongside this until the WS-14 cleanup phase retires the old shell.
...
/// Legacy three-mode preview seed — retained for the existing
/// `OneCalcShellApp` mount path until the WS-14 cleanup phase retires
/// the legacy shell.
...
mod tests {
    ...
    /// Pins the fixture shape of `preview_minimal_host_state` — the seed
    /// used by the WS-14 minimal home shell. Single empty formula space,
    /// no retained-artifact catalog.
    #[test]
