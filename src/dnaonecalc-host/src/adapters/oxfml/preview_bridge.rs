...
fn preview_document(request: &FormulaEditRequest) -> EditorDocument {
    ...
    let replacement_span = function_name.as_ref().and_then(|_| {
        inferred_function_replacement_span(&request.entered_text, request.cursor_offset)
    });
    let completion_proposals = function_name
        ...
        .unwrap_or_default();
    let signature_help = function_name
        .clone()
        .map(|callee_text| SignatureHelpContext {
            callee_text,
            call_span: FormulaTextSpan {
                start: 0,
                len: request.entered_text.chars().count(),
            },
            active_argument_index: active_argument_index(
                &request.entered_text,
                request.cursor_offset,
            ),
        });
    let function_help = function_name.as_ref().map(|lookup_key| FunctionHelpPacket {
